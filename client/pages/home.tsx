    ...
    <div>
      <div className='my-20'>
        <div ref={heading.ref} className={`${heading.inView ? 'translate-y-[0vh] opacity-1' : 'translate-y-[10vh] opacity-0'} transition-all duration-1000 ease-out `}>
          <h4 className='text-xs lg:text-sm font-heading my-5 text-center w-full tracking-[10px] font-semibold'>COLLECTIBLE</h4>
          <h3 className='text-3xl lg:text-4xl my-5 tracking-wide w-full text-center capitalize font-extrabold'>Find collectible near you
          </h3>
        </div>
        <Map />
        <div id='tours' ref={cards.ref} className={`flex flex-wrap justify-center transition-all duration-[1500ms] ease-out ${cards.inView ? 'opacity-100' : 'opacity-0'} `}>
          {eventsPassport.map((event, index) => (
            <CardItem icon={<LandscapeIcon />} modalDesc={passport.description} title={passport.name} desc={"lat:" + event.location.lat + " , lng:" + event.location.lng} img={event.image_url} />
          ))}
        </div>
      </div>
    </div>
