...
const CardModal = (props: Props) => {
  ...
  const qrCode = "https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=" + encodeURIComponent(props.img)
  ...
  return (
    <>
      ...
      <Modal
        ...
        onClose={handleClose}
      >
        <div className={`absolute top-[50px] left-[${isMobile ? '1%' : '10%'}] lg:left-[34%] shadow-xl w-[${isMobile ? '38vw' : '30vw'}] lg:w-[${isMobile ? '40vw' : '30vw'}] bg-white rounded-xl`}>
          <Image src={props.img} layout='responsive' objectFit='cover' height={'100%'} width={'100%'} alt={props.subtitle} />
          <div className='p-4'>
          ...
          <div className='flex justify-between items-center p-1 sm:p-4 bg-primary/70 text-white font-medium text-[10px] sm:text-xs md:text-sm lg:text-md md:rounded-tl-full capitalize font-heading'>
            ...
            <div className='w-24 h-24 relative'> {/* Adjusted width and height */}
              <Image
                src={qrCode}
                alt={qrCode}
                layout="fill"
                ...
                loading="lazy"
                onError={(e) => {
                  console.error('Image failed to load:', qrCode, e);
